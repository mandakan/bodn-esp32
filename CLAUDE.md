...
- **Display DMA budget**: the `_spidma` module sends display data in 32 KB DMA chunks at 80 MHz SPI (configurable via `TFT_SPI_BAUDRATE` in `config.py`). The last chunk is async (Python returns immediately), all earlier chunks block. **If a dirty rect fits in one chunk (≤50 full-width rows on the primary display), the push costs only ~1 ms.** Each additional chunk adds ~3 ms of blocking at 80 MHz. Design animations to stay within a narrow band (≤50 rows) whenever possible — see §3.0 in the performance guidelines. If displays show glitches (corrupted pixels, colour shifts), lower `TFT_SPI_BAUDRATE` to `40_000_000` — no firmware rebuild needed.

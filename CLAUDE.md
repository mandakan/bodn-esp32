...
- **TTS**: two TTS pipelines generate spoken audio offline with Piper TTS:
  ...
  - i18n TTS is converted to device format (16 kHz mono PCM) by `tools/convert_audio.py`. Story packages and i18n TTS are synced to the SD card by `tools/sd-sync.py`. On the device both go through `bodn.assets.resolve_voice(tts_path)` — `from bodn.tts import say` for i18n, `/stories/{id}/tts/{lang}/{node}.wav` lookup inside `bodn/ui/story.py` for stories.
- **Hand-recorded overrides**: any TTS line can be replaced with a human recording without touching code. Drop a WAV at `assets/audio/source/recordings/{lang}/{key}.wav` (i18n) or `assets/stories/{id}/recordings/{lang}/{node}.wav` (story — use the `_choices` suffix for choice narration). Filenames must match the TTS key/node exactly. `convert_audio.py` normalises the recording to 16 kHz mono PCM with loudnorm (same as TTS) into `build/sounds/recordings/…` or `build/stories/{id}/recordings/…` — both land on SD via the existing `sd-sync.py` mappings, no extra config needed. `bodn.assets.resolve_voice()` resolves `recording > TTS` at each storage layer (SD first, then flash), so coverage is incremental — record one key, the rest stay on TTS. **Footgun:** if you change the source text after recording, the old recording silently shadows the regenerated TTS — delete (or re-record) the affected file.
